...
extern "C" void draw_detections_cv_v3(mat_cv* mat, detection *dets, int num, float thresh, char **names, image **alphabet, int classes, int ext_output)
{
    try {
        ...
        for (i = 0; i < num; ++i) {
            ...
            if (class_id == 0) {
                cv::Mat img = cv::imread("./data/popup/university_headquarters.png"); 
                cv::imshow("university_headquarters", img);
                cv::waitKey();
            }
            else if (class_id == 1) {
                cv::Mat img = cv::imread("./data/popup/justice_hall.png");
                cv::imshow("justice_hall", img);
                cv::waitKey();
            }
            else if (class_id == 2) {
                cv::Mat img = cv::imread("./data/popup/cheongsong_hall.png");
                cv::imshow("cheongsong_hall", img);
                cv::waitKey();
            }
            else if (class_id == 3 || class_id == 8) {
                cv::Mat img = cv::imread("./data/popup/creation_hall.png");
                cv::imshow("creation_hall", img);
                cv::waitKey();
            }
            else if (class_id == 4) {
                cv::Mat img = cv::imread("./data/popup/baekwoon_hall.png");
                cv::imshow("baekwoon_hall", img);
                cv::waitKey();
            }
            else if (class_id == 5 || class_id == 12) {
                cv::Mat img = cv::imread("./data/popup/future_hall.png");
                cv::imshow("future_hall", img);
                cv::waitKey();

            }
            else if (class_id == 6 || class_id == 9) {
                cv::Mat img = cv::imread("./data/popup/central_library.png");
                cv::imshow("central_library", img);
                cv::waitKey();
            }
            else if (class_id == 7 || class_id == 10) {
                cv::Mat img = cv::imread("./data/popup/student_union_building.png");
                cv::imshow("student_union_building", img);
                cv::waitKey();
            }
            else if (class_id == 11) {
                cv::Mat img = cv::imread("./data/popup/eagle.png");
                cv::imshow("eagle", img);
                cv::waitKey();
            }            
        }
